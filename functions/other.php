...
$orangethemes_buttons=array("orangethemesbutton","orangethemessocial","orangethemesmarker", "orangethemesspacer", "orangethemesquote", "|",
			 "orangethemeslists", "orangethemestables", "orangethemestabs", "orangethemescountdown", "orangethemesstatus", "|",
			 "orangethemesgallery", "orangethemescaption", "orangethemesvideo", "orangethemesmap", "|", "orangethemesparagraph",  "|","orangethemesbreak");
...
function OT_page_main_title() {
	global $s;
	if ( is_single() ) { $title = single_post_title('', false).' | '.get_bloginfo('name'); }      
	elseif ( is_home() || is_front_page() ) { $title = get_bloginfo('name').' | '.get_bloginfo('description'); }
	elseif ( is_page() ) { $title = single_post_title('', false).' | '.get_bloginfo('description'); }
	elseif ( is_search() ) { $title = get_bloginfo('name').' | Search results ' . esc_html($s); }
	elseif ( is_404() ) { $title = get_bloginfo('name').' | Page not found'; }
...
function orangethemes_comment($comment, $args, $depth) {
   ...
   ?>
	...
	<div class="comment-block">
			<span class="comment-avatar"><img src="<?php echo get_avatar_url(get_avatar( $comment, 60));?>"  alt="" title="" /></span>
			<div class="commment-content" id="comment-<?php comment_ID() ?>">
				<span class="right time-stamp"><span class="icon-text">&#128340;</span><?php printf(__(' %2$s, %1$s', THEME_NAME), get_comment_date("d.F Y"), get_comment_time("G:i"));?></span>
				<h3><a href="<?php if(comment_author_url()) { echo comment_author_url();} else { echo "#"; } ?>"><?php printf(__('%1$s', THEME_NAME), get_comment_author());?></a>
				<?php if($comment->user_id == get_the_author_meta('ID')) { ?>
					<font class="this-is-author"><?php _e("AUTHOR", THEME_NAME);?></font>
				<?php } ?>
				</h3>
	...
	<?php if($commentNr!=get_comments_number('0','1','%')) { ?>
